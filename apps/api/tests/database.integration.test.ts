...
describe("PostgreSQL application schema", () => {
  ...
  it("upgrades a nonempty Phase 2 database without losing its workspace", async () => {
  ...
  it("upgrades a nonempty Phase 3 database without changing conversation data", async () => {
    ...
    try {
      ...
      expect(generationTable.rows).toEqual([{ tableName: "generations" }]);
    } finally {
      await verificationPool.end();
    }
  });

  it("upgrades a nonempty Phase 5 database while preserving prior deferred deletion", async () => {
      ...
      await expect(
        verificationPool.query(`
          INSERT INTO generations (
            ...
            requested_tier, system_prompt_version, behavior_contract_version,
            model_policy_revision,
